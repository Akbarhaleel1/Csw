...
if (!process.env.S3_ACCESS_KEY || !process.env.S3_SECRET_ACCESS_KEY || !process.env.S3_REGION) {
...
const uploadS3Video = async (file: Express.Multer.File): Promise<any> => {
  const params = {
  ...
  return new Upload({ client: s3config, params })
    .done()
    .then((data) => {
...
const uploadS3Image = async (file: Express.Multer.File): Promise<any> => {
  const params = {
